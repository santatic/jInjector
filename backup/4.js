...
function AjaxSuccess(dt,successfs,errorfs,sleep){
	setTimeout(function(){
...
function AjaxComplete(dt,completefs,sleep){
	setTimeout(function(){
		if(VICTIM['request']=='get'){
			$.ajax({
				url: dt,
...
function AdminScanShowLog(type,link,status,tmp){
	var t ="";
	if(tmp){
		t = "\t";
	}
...
function AdminScanDir(count){
	if(SADMIN['dironly'] || SADMIN['404']){
		...
		AdminScanGetStatus(AdminDirList[count],AdminScanDir,count+1,0);
	}else{
		AdminScanGetStatus(AdminDirList[count-1]+AdminFileList[0],AdminScanFile,1,1);
	}
}
function AdminScanFile(count){
	if(AdminFileList[count] == "" || typeof AdminFileList[count] == "undefined"){
		...
		AdminScanGetStatus(AdminDirList[SADMIN['dirid']-1]+AdminFileList[count],AdminScanFile,count+1,1);
	}
}
//============================
